...
## Research context and implementation coverage

The main research analysis uses observed trait data, with trait-specific matching and pruning. It explores mixed traits, estimates phylogenetic signal across posterior trees, and compares metric behaviour under Brownian-motion simulations. Imputation, PCA, alternative phylogenies, and Delta belong to related analyses; geographic assemblages and phylogenetic diversity were earlier exploratory work.

The public code provides runnable examples of selected methods:

| Component | Public implementation |
| --- | --- |
| Trait curation and taxonomy | Described in the research overview; the example validates generated identifiers and trait types without distributing the research crosswalk or compilation routines. |
| Tree matching and pruning | Implemented for the synthetic tree and trait-specific observed entries. |
| Signal estimation | Observed-entry λ, K, and binary D on one synthetic phylogeny. Posterior-tree summaries and Delta are described research scope, not implemented here. |
| Brownian simulation | Used to create illustrative traits and the circular trait map. A separate exporter repeats `fastBM` simulations and λ/K fitting as a runnable illustration of the comparison method, using arbitrary example settings. |
| Imputation evaluation | Two random-forest fits and deliberately masked-value error assessment. |
| PCA and associations | Standardized PCA and Spearman correlations on completed synthetic traits. |
| Trait visualization | An independently simulated mixed-variable matrix, distributions, coverage map, continuous trait map and categorical tip map, generated by the methods and trait exporters. |
| Tree sensitivity | A separate exporter holds toy traits and topology fixed while perturbing branch lengths and refitting λ/K. It does not generate posterior trees or posterior intervals. |
| Alternative trees and geographic assemblages | Described as related or earlier exploratory work; not part of the distributed executable pipeline. |

The following sections describe the executable implementation.
...
The PCA gallery figure overlays `prcomp` rotation vectors multiplied by a single positive display factor chosen to fit the score panel. These arrows show loading directions with generic trait labels; they are not correlation vectors or a claim of a particular formal biplot scaling. Scores and variance fractions remain unchanged.

## Additional fitted-method illustrations

`scripts/export_signal_figures.R` independently generates a small coalescent tree, Brownian traits with varying added Gaussian noise, and median-thresholded two-state examples. It fits λ, K and D to these inputs. D uses 99 permutations; fitting warnings or non-finite estimates stop the export rather than substituting plausible-looking values.

The sensitivity experiment keeps generated tip values and topology fixed and applies positive lognormal multipliers to branch lengths. The altered trees need not be ultrametric. Boxplots summarize this arbitrary perturbation experiment, not posterior uncertainty. Diamonds identify the unchanged-tree fit.

The Brownian experiment generates 36 independent `fastBM` traits on the unchanged simulated tree, then estimates λ and K for each replicate. One is the model reference, not a threshold determining whether a study trait passes a test. All sample sizes, noise levels and repetition counts are chosen for a quick illustration and are unrelated to research settings.
...
The repository landing page presents eight independently simulated PNGs from the methods, trait and signal exporters, plus the synthetic imputation and PCA SVGs. `R/gallery_plots.R` provides five reviewed SVG illustrations; `scripts/export_gallery.R` regenerates those from the workflow objects and adds accessible provenance descriptions. Its `--local-only` option renders the gallery under ignored `artifacts/` for CI without changing published SVG copies. See the [figure notes](workflow-outputs.md) for the plot mapping. Empirical figures and generated numerical tables are excluded from publication.
...
The core packages are [ape](https://cran.r-project.org/package=ape), [phytools](https://cran.r-project.org/package=phytools), [missForest](https://cran.r-project.org/package=missForest), and [caper](https://cran.r-project.org/package=caper). The mixed-trait illustration additionally uses GGally and ggplot2. Use `citation("package-name")` in the environment used for an analysis to retrieve its software citation.
