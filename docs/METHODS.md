...
| Brownian simulation | Used to create illustrative continuous traits and the circular trait map. The research's repeated λ/K simulation comparison is not reproduced by this example. |
...
| Mixed-trait visualization | A separately simulated mixed-variable matrix and continuous trait map, generated by `scripts/export_methods.R`. |
...
The repository landing page presents two independently simulated methods illustrations rendered by `scripts/export_methods.R`. The accompanying website also presents five synthetic SVG illustrations rendered by `R/gallery_plots.R`; `scripts/export_gallery.R` regenerates those from the workflow objects and adds accessible provenance descriptions. See the [figure notes](workflow-outputs.md) for the plot mapping. Empirical figures and generated numerical tables are excluded from publication.
